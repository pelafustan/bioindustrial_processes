\documentclass[a4paper,12pt]{article}
\usepackage[utf8]{inputenc}
\usepackage[spanish,es-tabla,es-lcroman]{babel}
\usepackage[T1]{fontenc}
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{graphicx}
\usepackage[left=1in,right=1in,top=0.5in,bottom=1in,includehead,includefoot]{geometry}
\graphicspath{{./images/}}
\usepackage{float}
\usepackage{booktabs}
\usepackage{array}
\usepackage{csquotes}
\usepackage{multicol}
\usepackage{hyperref}
\hypersetup{
  colorlinks=true, 
  pdftitle={Reporte Variables ambientales sobre el crecimiento microbiano}, 
  pdfauthor={Nicolás Garrido and Patricio Parada},
}
\usepackage{xpatch}
\usepackage{siunitx}
\usepackage[font=small]{caption}
\usepackage{multirow}
\usepackage{pgfplots}
\usepackage{pgfplotstable}
\pgfplotsset{compat=newest}
\usepackage[backend=biber,style=apa,citestyle=authoryear]{biblatex}
\addbibresource{reference.bib}

\DeclareSIUnit\ufc{UFC}
\DeclareSIUnit\gen{gen}

\setlength{\parindent}{0pt}
\setlength{\parskip}{6pt}

\decimalpoint

\begin{document}
  \begin{titlepage}
    \centering
    \rule[1ex]{\textwidth}{0.8pt}

    \vspace{1cm}
    \begin{minipage}[b]{.45\linewidth}
      \begin{figure}[H]
        \raggedright
        \includegraphics[height=2.0cm]{icb_logo}
      \end{figure}
    \end{minipage}
    \begin{minipage}[b]{.45\linewidth}
      \begin{figure}[H]
        \raggedleft
        \includegraphics[height=2.0cm]{utalca_logo}
      \end{figure}
    \end{minipage}

    \vspace{2cm}

    {\large \scshape Reporte 1 \par}
    \vspace{1cm}
    {\Large \bfseries Variables ambientales sobre el crecimiento microbiano \par}
    \vspace{2cm}
    {\itshape Nicolás Garrido Calderara \par}
    {\itshape J. Patricio Parada Gutiérrez \par}
    \vspace{2cm}

    \begin{tabular}{ll}
      \textbf{Coordinador}: & Dr. José Antonio Reyes Suárez \\
      \textbf{Encargado trabajos prácticos:} & Yerko Argandoña Vargas \\
    \end{tabular}

    \vfill
    {\scshape Facultad de Ingeniería \par}
    {\scshape Escuela de Ingeniería Civil en Bioinformática \par}
    {\scshape Procesos Bioindustriales \par}

    \vfill
    {\scshape \today \par}

    \vfill
    \rule{\textwidth}{0.8pt}

  \end{titlepage}

  \section{Introducción}

  \input{01_intro.tex}

  \newpage
  \section{Materiales y métodos}

  \input{02_materials.tex}

  \newpage
  \section{Resultados}

  \input{03_results.tex}
  
  \newpage
  \section{Discusión}

  \input{04_discussion.tex}

  \newpage
  \section{Conclusión} 

  \input{05_conclusion.tex}

  \newpage
  \printbibliography
  
\end{document}
